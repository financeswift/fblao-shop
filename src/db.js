...
function seed() {
  ...
  const defaults = {
    ...
    swiftpay_enabled: process.env.SWIFTPAY_ENABLED === '1' || !!(process.env.SWIFTPAY_API_KEY || process.env.SWIFTPAY_ACCESS_KEY) ? '1' : '0',
    swiftpay_mode: process.env.SWIFTPAY_MODE || 'sandbox',
    swiftpay_api_base_url: process.env.SWIFTPAY_API_BASE_URL || '',
    swiftpay_api_key: process.env.SWIFTPAY_API_KEY || process.env.SWIFTPAY_ACCESS_KEY || '',
